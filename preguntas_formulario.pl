/* Objeto = ingredientes, alergias, nivel. Frases de preguntas del formulario "¿me dices tus ingredientes favoritos?" */
frase_pregunta_formulario(F, Objeto) :- sn(S), sv(S, V, Objeto), F = [S, V, '?'].
...
sv(S, V, Objeto) :- pronombre_personal(S, P), objeto(G, N, Objeto), determinante(G, N, Det), sustituye_cd(Det, Sdet), verbo(Vb), V = [P, Sdet, Vb].
...
sustituye_cd(el, lo).
sustituye_cd(los, los).
sustituye_cd(las, las).
